...
def schwarzEnergy(a,p,e,x):
    ...
    return  np.sqrt((-4 *e*e + (-2 + p)*(-2 + p))/(p* (-3 - e*e + p)))

def schwarzMomentum(a,p,e,x):return (p*x)/np.sqrt(-3 - e*e + p)
    ...
    return np.sqrt(1 - \
                     ((1 - e*e)* \
                       ...
                       ((-1 + e*e)* \
                                                (a*a *(1 + 3 *e*e + p) + p* \
                                                                          (-3 - e*e + p - 2* x* \
                                                                                                     np.sqrt((a**6 *(-1 + e*e)*(-1 + e*e) + a*a *(-4 *e*e + (-2 + p)*(-2 + p)) *p*p + 2 *a*a*a*a *p *(-2 + p + e*e *(2 + p))
                              )/(p*p*p *x*x)
                          )
                         )
                        )
...
def circEquatEnergy(a,p,e,x):return ((-2 + p)* np.sqrt(p) + a/x) \
                                  /np.sqrt(2* a/x *p**(3/2) + (-3 + p)* p*p)

def circEquatMomentum(a,p,e,x):return (a*a - 2 *a/x *np.sqrt(p) + p*p) \
                                    /np.sqrt(2 *a/x + (-3 + p)* np.sqrt(p)) \
                                    /p**(3/4)
...
def spherEnergy(a,p,e,x):
    return np.sqrt(
        (
            ...
            np.sqrt(p*p*p + a*a *p *(-1 + x*x)) + a*a*a*a *p*p *(-1 + x*x) * \
...
def spherMomentum(a,p,e,x,En):
    ...
    return (-En *g + x *np.sqrt((-d *h + En*En *(g*g + f *h))/(x*x)))/h
...
def genE(a,p,e,x):# I somehow broke this when trying to speed it up.
    ...
    Eta = f1* g2 - g1* f2
    Sigma = g1* h2 - h1* g2
    return np.sqrt((Kappa * Rho + 2* Epsilon * Sigma - x * 2* np.sqrt(Sigma * (Sigma * Epsilon*Epsilon + Rho * Epsilon * Kappa - Eta * Kappa*Kappa)/(x*x)))/(Rho*Rho + 4* Eta * Sigma))
...
def KerrGeoRadialRoots(a, p, e, x, En= None, Q= None):
    ...
    r3 = (AplusB + np.sqrt(AplusB*AplusB - 4*AB))/2# (*Eq.(11)*)
    r4 = AB/r3
    return [r1, r2, r3, r4]
...
def KerrGeoPolarRoots(a, p, e, x):
    ...
    zm = np.sqrt(1 - x*x)
    zp = np.sqrt((a*a *(1 - En*En) + L*L/(1 - zm*zm)))
    return [zp, zm]
...
#              if(x==0 && e1==0):return 1 + np.sqrt(3) + np.sqrt(3 + 2* Sqrt(3))# Polar ISSO in extremal case
...
#         + 2 *C0(ru).Q
#         + ru*ru * \        (-1 - ru *Beta(ru) +
#          np.sqrt(1 + Beta(ru) * \                   (C0(ru).L**2 + C0(ru).Q - a*a *Beta(ru) - 2 *ru * \                    (1 + ru *Beta(ru))
#                    )
#                   )
#         )
#     Beta2 = (ru) => :let B= Beta(ru)
#                          return ru * \                         (2 + ru *B - 2 * \                          np.sqrt(1 +B* \                                    (C0(ru).L**2 + C0(ru).Q - 2 *ru - a*a *B - 2 *ru*ru *B
#                                     )
#                                    )
